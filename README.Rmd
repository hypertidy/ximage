...
missing functionality and usability problems fixed. Draw an image from
whatever you have, where you want it, with no dependencies beyond base R.
...
equivalent to populating an R matrix with `matrix(<>, nrow, ncol, byrow = TRUE)`
(note backwards order of nrow,ncol). Please note that getting the data out
of the matrix is not in this order, but it is with `as.vector(t(m))`.
Higher dimensional arrays need more care, there's no `byrow` for `array()`.
The full story is in the package vignette:
