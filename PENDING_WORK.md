...
This document catalogs all open issues and potential PRs that were
...
**Description**: Requires handling alpha channel consistently for RGB
images  
**Example Code**: Available in issue description showing hillshade
overlay use case  
...
**Type**: Documentation  
**Description**: Documentation issue explaining raster order vs matrix
order  
**Action**: Could be enhanced with additional examples or moved to
vignette  
...
**Description**: Create a scales::alpha() for arrays, hex, rgb,
nativeRaster that doesn’t drop dimension  
**Comments**: 1 comment  
**Estimated Effort**: Medium
...
**Description**: Error when handling NaN in RGB data from Sentinel
imagery  
...
**Comments**: 3 comments  
**Checklist**: - \[ \] Unpack raster/nativeraster to colours - \[ \]
Handle extent as 2D arrays for xy coords - \[ \] Handle corner cases
(for ROMS etc)  
...
8.  **PR for Issue \#8**: Implement extent calculations for angle
    parameter
...
- The previous Copilot session mentioned “more PRs to do” but specific
  details were not found in commit history
- This document serves as a comprehensive roadmap based on all open
  issues
- Each issue can be converted into a focused PR following the priority
  order above
- Consider grouping related issues (e.g., alpha-related issues) into
  single PRs if appropriate

## Next Steps

To continue from the lost session: 1. Review this roadmap with the team
2. Select the next issue to address based on priority 3. Create a
focused PR for that issue 4. Update this document as issues are resolved
